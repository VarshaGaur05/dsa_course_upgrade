...
public class FindSymmetricPairs {
    public static void main(String arg[]) {
        ...
        for (int i = 0; i<arr.length; i++){
            ...
            int first1 = arr[i][0]; //first element of the current pair
            int second1 = arr[i][1]; //second element of the current pair
         // check if the scond element of the current pair is present in the hashmap or not

            Integer data = hashMap.get(second1);

            //If present and the firstC element of the current pair and value of the

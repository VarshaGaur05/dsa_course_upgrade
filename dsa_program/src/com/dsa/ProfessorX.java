...
public class ProfessorX {
    public static void main(String ss[]) {
        ...
        sortFirstNameAscending(firstNames);
        sortLastNameDescending(lastNames);
    }

    private static void sortFirstNameAscending(String[] firstNames) {
        for(int i = 1; i<firstNames.length;i++) {
            int j = i-1;
            String key = firstNames[i];
            while(j>=0) {
                if(key.compareTo(firstNames[j]) > 0) {
                    break;
                }
    ...
    private static void sortLastNameDescending(String[] lastNames) {
        for(int i = 1; i<lastNames.length;i++) {
            int j = i-1;
            String key = lastNames[i];
            while(j>=0) {
                if(key.compareTo(lastNames[j]) < 0) {
                    break;
                }
